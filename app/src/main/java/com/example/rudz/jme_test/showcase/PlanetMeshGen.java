...
public class PlanetMeshGen {
    ...
    public Mesh generateMesh(float radius) {
        ...
        // Set buffers
        mesh.setBuffer(VertexBuffer.Type.Position, 3, BufferUtils.createFloatBuffer(vertexList.toArray(new Vector3f[0])));
        mesh.setBuffer(VertexBuffer.Type.Normal, 3, BufferUtils.createFloatBuffer(normalList.toArray(new Vector3f[0])));
        mesh.setBuffer(VertexBuffer.Type.Index, 1, BufferUtils.createIntBuffer(toIntArray(indexList)));
        mesh.setBuffer(VertexBuffer.Type.Color, 4, BufferUtils.createFloatBuffer(toFloatArray(colorList)));
        mesh.updateBound();
     ...
     *
     * @param width
     *         The width of the heightmap. Larger values mean more complex mesh
     * @param seed
     *         The random seed for generating the heightmap
     * @param numIslands
     *         Total number of land masses
     * @param islandRadius
     *         How big each land mass is
     * @param iterations
     *         More interation equals more complex features
     * @param displacement
     *         How high land features get
     * @param smoothing
     *         Lower numbers mean smoother land features
     */
